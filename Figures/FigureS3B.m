...
cop = 0.46; % target [Na+] at MD
n = 2; % slope of the sigmoid function
Fg4 = alpha*(cop^n./(cop^n+c.^n)-beta);

cop = 0.3; % target [Na+] at MD
n = 4; % slope of the sigmoid function
Fg5 = alpha*(cop^n./(cop^n+c.^n)-beta);

figure(1)
plot(conc,Fg3(:),'k',"LineWidth",3); hold on
plot(conc,Fg4(:),'r',"LineWidth",3)
plot(conc,Fg5(:),'r:',"LineWidth",3)
ylabel('\DeltaGFR / \Deltat','fontsize',16)
xlabel('[Na+]_{MD} (mM)','fontsize',16)
axis([0 120 -0.25 0.25])
yline(0)
legend('Baseline','Furosemide-HD','Furosemide-LD')
hold off
